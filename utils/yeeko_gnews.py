...
class YeekoGoogleNews:
    ...
    def __ceid(self):
        ...
        lang = self.lang
        return f'?ceid={self.country}:{lang}&hl={lang}&gl={self.country}'
    ...
    def __scaping_bee_request(self, api_key, url):
        ...
        if response.status_code == 200:
            return response
        if response.status_code != 200:
            txt = "ScrapingBee status_code"
            raise Exception(f"{txt}: {response.status_code} {response.text}")
    ...
    def topic_headlines(self, topic: str, proxies=None, scraping_bee=None):
        ...
        if topic.upper() in [
            ...
            base_url = f'{self.BASE_URL}/headlines/section/topic/{topic.upper()}'
            d = self.__parse_feed(
                base_url + self.__ceid(),
                proxies=proxies,
                scraping_bee=scraping_bee
            )

        else:
            base_url = f'{self.BASE_URL}/topics/{topic}'
            d = self.__parse_feed(
    ...
    def geo_headlines(self, geo: str, proxies=None, scraping_bee=None):
        ...
        given a country and a language"""
        base_url = f'{self.BASE_URL}/headlines/section/geo/{geo}'
        d = self.__parse_feed(
            base_url + self.__ceid(),
            proxies=proxies,
            scraping_bee=scraping_bee
    ...
    def search(
            self, query: str, helper=True, when=None, from_=None, to_=None,
            proxies=None, scraping_bee=None):
        # proxies=None, scraping_bee=None, negative_words: list = None):
        """
        ...
        if when:
            # query += ' when:' + when
            dates_str += f' when:{when}'

        if from_ and not when:
            from_ = self.__from_to_helper(validate=from_)
            # query += ' after:' + from_
            dates_str += f' after:{from_}'

        if to_ and not when:
            to_ = self.__from_to_helper(validate=to_)
            # query += ' before:' + to_
            dates_str += f' before:{to_}'
        # replace {{DATE}} with the date string
        query = query.replace('{{DATE}}', dates_str)

        # if negative_words:
        #     query += ' -' + ' -'.join(negative_words)
        ...
        base_url = f'{self.BASE_URL}/search?q={query}'
        print("full url:", base_url + search_ceid, "\n")
        # return None
        d = self.__parse_feed(
            base_url + search_ceid,
            proxies=proxies,
            scraping_bee=scraping_bee
        )
